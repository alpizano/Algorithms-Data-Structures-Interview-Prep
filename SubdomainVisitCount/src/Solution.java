		...
		for (int i = 0; i <= cpdomains.length - 1; i++) {
			...
			for (int j = 0; j <= cpdomains.length - 1; j++) {
				if (i != j) {
					...
					if (subdomain_control_padded[2].equals(subdomain_compare_padded[2])
							&& subdomain_control_padded[1].equals(subdomain_compare_padded[1])) {
						count_arr[1] = count_arr[1] + domain_count_compare;
					}
					if (subdomain_control_padded[2].equals(subdomain_compare_padded[2])
							&& subdomain_control_padded[1].equals(subdomain_compare_padded[1])
							&& subdomain_control_padded[0].equals(subdomain_compare_padded[0])) {
			...
			set.add(String.valueOf(count_arr[1]) + " " + subdomain_control_padded[1] + "."
					+ subdomain_control_padded[2]);

			if (!subdomain_control_padded[0].equals(" ")) {
				set.add(String.valueOf(count_arr[0]) + " " + subdomain_control_padded[0] + "."
						+ subdomain_control_padded[1] + "." + subdomain_control_padded[2]);
			}
		} // end outer loop
